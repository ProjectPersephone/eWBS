mainApp.controller("StoryTaskController", function($scope, $cookieStore,HttpService) {
	...
	$scope.loadStoryIds = function() {
		HttpService.get("story/list/" + $scope.projectName).success(
				function(data) {
	...
	$scope.save = function() {
		...
		HttpService.post("storytask/save", $scope.storyTask).success(
				function(data) {
					$scope.load();
	...
	$scope.$watch('storyTask.effortActual+storyTask.effortPlanned+storyTask.workCompletedPer',function() {
	...
	$scope.$watch('storyTask.actualEndDate+storyTask.plannedEndDate+storyTask.plannedStartDate',function() {
				var storyTask = $scope.storyTask;
				storyTask.scheduleVariance = ($scope.dateDifference(
						storyTask.actualEndDate,
						storyTask.plannedEndDate) / (($scope.dateDifference(
